...
const TokenList = React.memo(() => {
  ...
  const [socketUrl] = React.useState('wss://stream.coinmarketcap.com/price/latest');
  const [socketSubscribed, setSocketSubscribed] = React.useState(false);
  ...
  const [{data}, getData] = useLazyFetch<{
  ...
  React.useEffect(() => {
    ...
    if (socketSubscribed && !tokens.length) {
    ...
    if (tokens.length) {
      ...
      sendMessage(JSON.stringify({
        ...
        id: 'price',
        data: {
          cryptoIds: tokens.flatMap(t => t.cmcId),
          index: 'detail'
        }
      }));
    }
  ...
  React.useEffect(() => {
    ...
    switch (lastMessage.data) {
      ...
      default: {
        try {
          const {id, d: {cr}} = JSON.parse(lastMessage.data);
          switch (id) {
            case 'price': {
              setTableData(prev => {
            ...
            (
              <TableContent>
                {tableData.tokens?.sort((a, b) => {
                  ...
                  return (
                    <TableRowLink key={token.id} to={`/token/${network}/${token.id}`}>
