...
export const CurrentCoinData = createContext<
  CoinMainPage | undefined | null
>(undefined)
...
export const CoinPage: FC = React.memo(() => {
  ...
  React.useEffect(() => {
    ...
    setToken({
      circulation_supply:
        data.statistics.circulatingSupply ||
        toFixedToken(data.selfReportedCirculatingSupply, 5),
      daily_volume: data.volume,
      daily_volume_change: data.volumeChangePercentage24h,
      fully_diluted_mc:
        data.statistics.fullyDilutedMarketCap,
      fully_diluted_mc_change:
        data.statistics
      ...
      market_cap:
        data.statistics.marketCap ||
        (data.selfReportedCirculatingSupply
          ? Number(data.selfReportedCirculatingSupply) *
            data.statistics.price
          : undefined),
      name: data.name,
      platforms: data.platforms,
      price_btc: Number.parseFloat(
        data.statistics.priceBtc,
      ),
      price_change_1h:
        data.statistics.priceChangePercentage1h,
      price_change_7d:
        data.statistics.priceChangePercentage7d,
      price_change_24h:
        data.statistics.priceChangePercentage24h,
      price_change_btc:
        data.statistics.priceBtcChangePercentage24h,
      price_change_eth:
        data.statistics.priceEthChangePercentage24h,
      price_change_usd:
        data.statistics.priceChangePercentage24h,
      price_eth: Number.parseFloat(
        data.statistics.priceEth,
      ),
      price_usd: data.statistics.price,
      total_supply: data.statistics.totalSupply,
      dateLaunched: data.dateLaunched
        ? new Date(data.dateLaunched)
        : data.dateAdded
